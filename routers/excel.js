...
const { checkLogin, checkCompany } = require("../middlewares");
...
router.post("/get_sheets",async (req, res)=>{
        ...
        if (connection !== null)
        {
            ...
            try{
                let result = await connection.db("nextERP").collection("sheets").find({companyId: req.user.companyId, doc_id: new ObjectId(doc_id)}).toArray();
                res.send(result);
            }catch(e){
                console.log(e);
...
router.post("/get_all_sheets", async (req, res)=>{
    ...
    let response = await excel_manager.get_all_sheets(user.companyId);
...
router.post("/add_sheet",async (req, res)=>{
    ...
    if (doc_id !== undefined)
    {
        let doc_response = await excel_manager.getDocumentById(doc_id, req.user.companyId);
...
router.post("/removeSheet",async (req, res)=>{
    ...
    if (sheetId !== undefined)
    {
        let response = await excel_manager.remove_sheet(sheetId, req.user.companyId);
        if (response?.success === true){
            res.sendStatus(200);
        }else{
            res.status(500).send(response);
...
router.post("/add_document", async (req, res)=>{
    ...
    if (color !== undefined && name !== undefined && page_id !== undefined)
    {
        ...
        {
            let response = await excel_manager.addDocument(name, color, page_id, req.user.companyId);
            if (response?.success === true){
                res.send(response);
            }else{
                res.status(500).send(response);
...
router.post("/remove_document", async (req, res)=>{
    ...
    if (doc_id !== undefined)
    {
        let response = await excel_manager.remove_document(doc_id, req.user.companyId);
        if (response?.success === true){
            res.sendStatus(200);
        }else{
            res.sendStatus(500);
...
router.post("/rename_document", async (req, res)=>{
    ...
    {
        let response = await excel_manager.rename_document(doc_id, req.user.companyId, name);
        if (response?.success === true)
        {
            res.sendStatus(200);
        }else{
...
router.post("/import_excel_final", file_import.single("file"), async(req, res)=>{
    ...
    {   
        try{
            ...
            let response = await excel_manager.import_excel(data, sheetId, doc_id, req.user.companyId, req.file.path);
