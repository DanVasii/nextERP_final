...
router.get("/", parse_favourites, async (req, res)=>{
    let menu = await document_manager.getPagesAndDocuments(req.user.companyId);
    let docsForPage = [];

    if (menu?.success === true){
        res.render("reporting.html",{pages: menu.result, ...req.user, favourites: req.favourites});
    }else{
        res.sendStatus(404);
    }
})
...
router.post("/save_diagram", async (req, res)=>{
    ...
    if (name !== undefined && json !== undefined && image !== undefined && sheets_included !== undefined){
        ...
        let response = await reporting_manager.save_diagram(name, json, req.user.companyId, image, sheets_included);
        res.send(response);
...
router.get("/get_diagrams", async (req, res)=>{
    ...
    let diagram_response = await reporting_manager.get_diagrams(req.user.companyId);
...
router.post("/relational_data", async (req, res)=>{
    let {sheets, selected, relations, main_sheet, main_sheet_data} = req.body;


    if (sheets !== undefined && selected !== undefined && relations !== undefined && main_sheet !== undefined && main_sheet_data !== undefined){
        let response = await reporting_manager.get_relational_data(sheets, selected, relations, main_sheet, main_sheet_data);
        res.send(response);
    }else{
        res.status(500).send(ERRORS.INCOMPLETE_REQUEST);
    }
