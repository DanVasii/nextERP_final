var custom_select, sheetNames = null;
    ...
    $.ajax({
        ...
        success: (data)=>{
            if (data?.success === true){
                ...
                populate_type_two__relational(relation_sheets, parentNode, data.data);
...
function get_main_sheet(node){
    return custom_select.getPickedOption(node.querySelector("#table_select"));
}

function render_type_two__relational(sheet_names, data, node){
    let frag = document.createDocumentFragment();

    const get_sheetName = (sheet_id)=>{
        for (const sheet_object of sheet_names){
            if (sheet_object._id == sheet_id)
            {
                return sheet_object.sheetName;
            }
        }
    }

    for (const sheet_id of Object.keys(data)){

        let container = document.createElement("div");
        container.className = "reporting_body--table_container";

        container.appendChild(special_table_node(data[sheet_id], sheet_id, get_sheetName(sheet_id)));

        //build the graph page 
        let graph_page = document.createDocumentFragment();
        //add the tray
        let column_tray = document.querySelector("#column_tray").content.cloneNode(true);

        if (data[sheet_id].length != 0){
            data[sheet_id][0].data.forEach((cell)=>{
                let div = document.createElement("div");
                div.className = "reporting_column white";
                div.dataset.uuid = cell.uuid;

                //WORKING HERE
                div.onclick = (e)=>{
                    let target = e.target;
                    let container = target.parentNode.parentNode.parentNode;
                    //remove old canvas
                    let canvas; 
                    
                    if ((canvas = container.querySelector("canvas")))
                        canvas.remove();

                    canvas = document.createElement('canvas');
                    container.appendChild(canvas);

                   create_graph(cell.colType, raw_data_to_1D(data[sheet_id], target.dataset.uuid), canvas, cell.colName);
                }

                div.textContent = cell.colName;
                column_tray.querySelector(".reporting--column_tray").appendChild(div);
            })
        }

        graph_page.appendChild(column_tray);

        let views = new view_selector(["Raw data","Graphic"],[container, graph_page], ["table", "graph"], true, {"width": "100%"});
        views.style.marginTop = "10px";

        frag.appendChild(views);
    }

    node.querySelector(".reporting_body--extras_table").appendChild(frag);
}

function raw_data_to_1D(data, colUuid){
    let returned_data = [];
    data.forEach((row)=>{
        row.data.forEach((cells)=>{
            if (cells.uuid == colUuid)
            {
                if (row._count !== undefined)
                cells._count = row._count;

                returned_data.push(cells);
            }
        })
    })
...
function special_table_node(data, sheet_id, sheetName = undefined){
    let node = document.createElement("table");
    node.dataset.page = 1;
    node.dataset.for = sheet_id;
    node.className = "special_table";
    node.dataset.sheetname = sheetName;

    //sort data by the rowId
    ...
    data[0].data.forEach((cell)=>{
        colOrder.push(cell.uuid);

        td = document.createElement('th');
        td.dataset.uuid = cell.uuid;
        td.textContent = cell.colName;
        tr.appendChild(td);
    })
    node.appendChild(tr);
    ...
    data.forEach((row)=>{
        ...
        for (const ordered_uuid of colOrder)
        {
            for (const cell_data of row.data)
            {
                if(cell_data.uuid == ordered_uuid){
                    td = document.createElement("td");
                    td.textContent = cell_data.value;
                    tr.appendChild(td);
                    break;
                }
            }
...
function populate_type_two__relational(sheets, node, main_sheet_data){
    ...
    $.ajax({
        ...
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify({sheets, relations: get_relations(sheets),main_sheet: get_main_sheet(node), selected: get_selected_rows(node), main_sheet_data}),
        success: (data)=>{
            console.log("THIS")
            console.log(data);
            if (data?.success === true){
                if (sheetNames == null){
                    get_sheet_names(sheets).then((sheet_names)=>{
                        render_type_two__relational(sheet_names, data.data, node);
                    });
                }else{
                    render_type_two__relational(sheetNames, data.data, node);
                }
            }else{
                Toastify({
                    className: "toast_error",
                    text: "Error getting relational data!"
                }).showToast();
            }
...
function create_graph__string(data){
    ...
    data.forEach((data_object)=>{
        ...
        if(data_object._count !== undefined){
            labels.push(value);
            fake_labels.push(value);
            dataset.push(data_object._count);
        }
        else{
