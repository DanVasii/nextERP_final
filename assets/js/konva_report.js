...
function render_relation_menu(line_obj){
    ...
    parentNode.querySelector("#remove_line").onclick = ()=>{
        remove_line(line_obj);
        showMenuPage("general");
    }
...
function remove_line(line_obj){
    if (lines.indexOf(line_obj) != -1){
        line_obj.line.remove();
        line_obj.text.remove();

        lines.splice(lines.indexOf(line_obj), 1);
    }
}
...
function get_column_shape_by_id(id){
    ...
    for (const sheet_group of stage.children[0].children){
        ...
        if (sheet_group.children !== undefined)
