...
async function sheetExists_id(sheetId, companyId)
{
    let connection = await mongo.getConnection();

    if (connection){
        try{
            let response = await connection.db("nextERP").collection("sheets").findOne({_id: new ObjectId(sheetId), companyId: companyId});
...
async function getDocumentsForUser(companyId)
{
    let connection = await mongo.getConnection();
    ...
    {   
        try{
            let results = await connection.db("nextERP").collection("documents").find({companyId: companyId}).toArray();
...
async function getDocumentById(doc_id, companyId)
{
    let connection = await mongo.getConnection();
    ...
    {
        try{
            let result = await connection.db("nextERP").collection("documents").findOne({_id: new ObjectId(doc_id), companyId: companyId});
...
async function getDocument(doc_name, companyId, conn = null)
{
    let connection = conn || await mongo.getConnection();
    ...
    {   
        try{
            let results = await connection.db("nextERP").collection("documents").findOne({companyId: companyId, name: doc_name});
            if (results !== null){
                return {success: true, data: results}
            }
            return {success: false};
...
async function addDocument(doc_name, doc_color, page_id, companyId)
{
    let connection = await mongo.getConnection();
    if (connection !== null){
        let docExists = await getDocument(doc_name, companyId);
        console.log(docExists);
        if (docExists?.success === false)
        {
            //insert it now 
            try{
                let insert = await connection.db("nextERP").collection("documents").insertOne({name: doc_name, color: doc_color, companyId: companyId});
                if (insert){
            ...
            if (cell_data !== null){
                ...
                if (hyperlink_coords && hyperlink_coords.trim() != ""){
                    ...
                    let colData = "";
                        ...
                        let docData = await getDocumentById(sheetData.data.doc_id, sheetData.data.companyId);
...
async function remove_document(doc_id, companyId)
{
    let connection = await mongo.getConnection();
    ...
    if (connection !== null)
    {
        try{
            ...
            let sheets = await connection.db("nextERP").collection("sheets").find({doc_id: new ObjectId(doc_id), companyId: parseInt(companyId)}).toArray();
            let response = await connection.db("nextERP").collection("documents").deleteOne({_id: new ObjectId(doc_id),companyId: parseInt(companyId)});

            if (response){
                try{
                    sheets.forEach((sheet)=>{
                        remove_sheet(sheet._id, companyId);
                    })
                }catch(e){}
...
async function remove_sheet(sheetId, companyId)
{
    let connection = await mongo.getConnection();
    ...
    {
        try{
            let sheets = await connection.db("nextERP").collection("sheets").deleteMany({_id: new ObjectId(sheetId), companyId: companyId});
...
async function rename_document(doc_id, companyId, doc_name)
{
    let connection = await mongo.getConnection();
...
async function get_all_sheets(companyId)
{
    let connection = await mongo.getConnection();
    ...
    if (connection){
        try{
            const sheets = await connection.db("nextERP").collection("sheets").aggregate([
                {
                  $match: {
                    companyId: companyId // Match sheets with the provided companyId
                  }
                },
                {
                  $lookup: {
...
async function import_excel(data, sheetId, doc_id, companyId, file_path)
{
    let connection = await mongo.getConnection();
    if (connection)
    {
        try{
            //check the document 
            let doc_data = await getDocumentById(doc_id, companyId);
            ...
            if (doc_data?.success === true)
            {
                ...
                for (const [index, sheet] of data.sheets.entries())
                {
                    ...
                    }else if (sheet.type == 1)
                    {
                        //first create 
                        try{
                            let db_sheet = await createSheet(sheet.extraData.sheetName, "", companyId, doc_id);
                            if (db_sheet?.success === true)
                            {
                                ...
                                worksheet.getRow(1).eachCell((cell)=>{
                                    header_promises.push(
                                        createHeader(newsheetId, undefined, cell.value, "", "", "string", companyId)
                                    )
                                })
                    ...
                    else if (sheet.type == 2)
                    {
                        ...
                        for (const [colData_index, colData] of sheet.extraData.columnData.entries())
                        {
                            if (colData.checked === true)
                            {
                                ...
                                {
                                    //create 
                                    columns_promises.push(createHeader(new ObjectId(sheetId), undefined, colData.col_name, "", "", "string", companyId))
                                }else{
                                    //just get the column 
...
async function createSheet(name, notes, companyId, doc_id)
{
    let connection = await mongo.getConnection();
    ...
    {
        try{
            if (name.trim() != "" && companyId !== undefined && doc_id !== undefined){
                let doc_data = await getDocumentById(doc_id, companyId);
                if (doc_data?.success === true)
                {
...
async function createHeader(sheetId, uuid = undefined, name, notes, formula, colType, companyId)
{
    let connection = await mongo.getConnection();
    ...
    {
        try{
            let sheetData = await sheetExists_id(sheetId, companyId);
            if (sheetData?.success === true)
            {
                if (uuid === undefined){
                    uuid = uuidv4();
